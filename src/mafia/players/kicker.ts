...
import { cancelVote, startVote } from '../ui/prompt/kick-vote'
import userData from '../user/data'
import playersData from '../players/info'
import { onKickVote as onKickVote } from '../ui/prompt/kick-vote'
import find from '../utils/array/find'
import { onId } from '../players/id'
...
import { state } from '../scene/enter-leave'
...
kicker.on('kick-answer', async ({ userId, answer, from }) => {
  ...
  if (includes(no, from) || includes(yes, from)) {
    return
  }
...
async function onKickEnd({
  ...
  removeOne(activeVoting, (u) => u.whom === whom && u.initiator === initiator)
